...
async function getLaunchOptions(chromiumLauncher) {
  ...
  if (useBundled) {
    ...
    const serverlessArgs = IS_SERVERLESS ? [
    ...
    return {
      executablePath,
      args: [...(sparticuz.args || []), ...baseArgs, ...serverlessArgs],
...
async function waitForPageReady(page) {
  ...
  }, undefined, { timeout: IS_SERVERLESS ? 8_000 : 12_000 }).catch(() => {});
  await page.waitForTimeout(IS_SERVERLESS ? 400 : 700);
}
...
export async function htmlToFigmaSvg(html, {
  ...
  const attempts = IS_SERVERLESS ? 2 : 1;
  let lastErr;
  ...
  return withFigmaBrowser(chromium, async (browser) => {
    ...
    try {
      ...
      await page.setContent(cleaned, {
        ...
        timeout: IS_SERVERLESS ? 25_000 : 45_000,
      });
      ...
      const result = await page.evaluate(async (opts) => {
        ...
        viewportWidth,
        maxLayers: IS_SERVERLESS ? 3200 : 6000,
        format,
        name: title,
        route,
      });
