...
import { existsSync as existsSync3, mkdirSync as mkdirSync4, writeFileSync as writeFileSync3 } from "fs";
...
// src/fallbackAssets.ts
import { existsSync, mkdirSync as mkdirSync2, writeFileSync } from "fs";
import { join as join2 } from "path";
var PLACEHOLDER_IMAGE_WEB_PATH = "/_assets/__placeholder__.svg";
var PLACEHOLDER_IMAGE_FILENAME = "__placeholder__.svg";
var PLACEHOLDER_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="320" height="200" viewBox="0 0 320 200" role="img" aria-label="Image placeholder">
...
var PLACEHOLDER_IMAGE_BODY = Buffer.from(PLACEHOLDER_SVG, "utf8");
var FALLBACK_FONT_CSS = `
html, body {
  font-family: system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif;
}
`.trim();
function ensurePlaceholderAsset(assetsDir) {
  const filePath = join2(assetsDir, PLACEHOLDER_IMAGE_FILENAME);
...
var CAROUSEL_CONTAINER_SELECTOR = CAROUSEL_SKIP_SELECTOR;
var CAROUSEL_SLIDE_SELECTOR = [
  '[class*="slide"]',
  "[data-slide]",
  '[role="group"]',
  'li[class*="slide"]',
...
function normalizeStackedTextRotatorsInDocument() {
  ...
  document.querySelectorAll('.relative, [class*="relative"]').forEach((rel) => {
    if (seen.has(rel) || rel.closest?.('[style*="display: none"]')) return;
  ...
  parents.forEach((parent) => {
    ...
    if (children.length < 2) {
      Array.from(parent.children).forEach((wrap) => {
        ...
        const rects2 = nested.map((el) => el.getBoundingClientRect());
        let overlapPairs2 = 0;
        for (let i = 0; i < nested.length; i++) {
          for (let j = i + 1; j < nested.length; j++) {
            if (rectsOverlapHeavily(rects2[i], rects2[j])) overlapPairs2++;
          }
        }
        if (overlapPairs2 < 1) return;
...
async function revealNavDropdownLinks(page) {
  try {
    ...
    logger.debug(`  [NAV REVEAL WARN] ${err.message}`);
  }
}
...
function assetFetchHeaders(pageUrl) {
  let origin = "";
  try {
...
function decodeHtmlUrl(value) {
  return String(value || "").replace(/&amp;/gi, "&").replace(/&quot;/gi, '"').replace(/&#39;/g, "'").trim();
}
...
async function capturePage(context, pageUrl, assetsDir, hooks = {}) {
  ...
  async function notifyArtifactWritten(relPath, absPath) {
    if (!hooks.onArtifactWritten) return;
    try {
      await hooks.onArtifactWritten({ relPath, absPath, kind: "asset" });
    } catch (err) {
      logger.warn(`  [OFFLOAD] ${relPath}: ${err.message}`);
    }
  }
  ...
  try {
    ...
    domAssetUrls = [...new Set(
      domAssetUrls.map((u) => decodeHtmlUrl(u)).map((u) => {
        try {
          return preferLargestSrcsetCandidate(new URL(u, pageUrl).href);
        } catch {
          return u;
        }
      })
    ...
    for (const [, { localPath, cssText, sourceUrl }] of cssFilesForRewrite) {
      try {
        ...
        if (rewritten !== cssText) {
          writeFileSync2(localPath, rewritten, "utf8");
          cssRewritten++;
    ...
    if (moreDomAssets.length) {
      const normalizedMore = [...new Set(
        moreDomAssets.map((u) => decodeHtmlUrl(u)).map((u) => {
          try {
            return preferLargestSrcsetCandidate(new URL(u, pageUrl).href);
          } catch {
            return u;
    ...
    await page.evaluate(() => {
      const pickSrcFromSrcset = (srcset, fallback) => {
        ...
        const pick = parts.find((p) => /\b1920w\b/.test(p)) || parts.find((p) => /\b1080w\b/.test(p)) || parts.find((p) => /\b828w\b/.test(p)) || parts[parts.length - 1];
    ...
    const linkData = await page.evaluate((origin2) => {
      ...
      const navFound = /* @__PURE__ */ new Set();
      const push = (value, nav = false) => {
        if (!value) return;
        try {
          const href = new URL(value, window.location.href).href;
          if (href.startsWith(origin2)) {
            found.add(href);
    ...
    const links = linkData.links.map((link) => normalizePageUrl(link, pageUrl)).filter((link) => !!link && new URL(link).origin === origin);
    const navLinks = linkData.navLinks.map((link) => normalizePageUrl(link, pageUrl)).filter((link) => !!link && new URL(link).origin === origin);
...
// src/localePaths.ts
var LANG_CODES = /* @__PURE__ */ new Set([
  "aa",
  "ab",
  "ae",
  "af",
...
var REGION_CODES = /* @__PURE__ */ new Set([
  "ad",
  "ae",
  "af",
  "ag",
...
var NON_LOCALE_SHORT = /* @__PURE__ */ new Set([
  "ai",
  "api",
  "app",
  "apps",
  ...
  "my",
  "id"
  // 'id' can be Indonesia OR identity — keep if second segment exists
]);
var LOCALE_SEGMENT_RE = /^[a-z]{2}(?:-[a-z]{2,8})?$/i;
function pathSegments(pathname) {
  return String(pathname || "/").split("/").map((s) => s.trim()).filter(Boolean);
}
...
function shouldUseBundledChromium(playwrightPath, platform = process.platform, serverless = IS_SERVERLESS) {
  return serverless || platform === "linux" && !existsSync3(playwrightPath);
}
function systemBrowserChannel(playwrightPath, platform = process.platform, serverless = IS_SERVERLESS) {
  if (serverless || existsSync3(playwrightPath)) return void 0;
...
function prioritizeSitemapUrls(urls, startUrl, cap = SITEMAP_SEED_CAP) {
  ...
  return [...scored.entries()].sort((a, b) => b[1] - a[1]).slice(0, cap).map(([url]) => url);
}
async function fetchSitemap(origin) {
  ...
  try {
    const robots = await fetch(`${origin}/robots.txt`, {
      ...
      signal: AbortSignal.timeout(IS_SERVERLESS ? 2e3 : 5e3)
    });
    if (robots.ok) {
      ...
      for (const nestedUrl of nestedUrls.slice(0, IS_SERVERLESS ? 20 : 80)) {
        ...
        try {
          const res = await fetch(nestedUrl, {
            headers: { "User-Agent": USER_AGENT3 },
            signal: AbortSignal.timeout(IS_SERVERLESS ? 3e3 : 8e3)
          });
          if (!res.ok) continue;
...
function linkEnqueuePriority(url, fromNav = false) {
...
async function saveStaticAsset(rawUrl, pageUrl, assetsDir) {
  ...
  try {
    ...
    const localPath = join4(assetsDir, filename);
    const webPath = `/_assets/${filename}`;
...
async function crawl(opts, assetsDir, onPage, hooks = {}) {
  ...
  const enqueue = (url, currentDepth, fromNav = false) => {
    const clean = normalizePageUrl(url);
    if (!clean) return;
    try {
      if (new URL(clean).origin !== origin) return;
      ...
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => {
          timeoutHandle = setTimeout(() => {
            ...
            reject(new Error(`Page capture timed out after ${captureTimeout / 1e3}s`));
          }, captureTimeout);
        });
        let result;
        try {
...
var IMAGE_URL_ATTRS = /* @__PURE__ */ new Set([
  "src",
  "data-src",
  "data-lazy-src",
  "data-original",
...
function rewriteHtml(record, origin) {
  ...
  return injectFallbackStyles(serialize(doc));
}
...
async function generateNextApp(outDir, manifest, apiRoutes) {
  const { pages } = manifest;
  const hostname = new URL(manifest.targetOrigin).hostname;
  mkdirSync5(join5(outDir, "public", "_assets"), { recursive: true });
  const pagesDataDir = join5(outDir, "captured-pages");
  ...
  const routeMap = { ...existingRouteMap };
  for (const page of pages) {
    const name = safeName(page.route);
    const filename = existingRouteMap[page.route] || `${name}.html`;
    const pageHtmlPath = join5(pagesDataDir, filename);
    if (page.html && page.html.length > 0) {
      writeFileSync4(pageHtmlPath, page.html, "utf8");
    }
  ...
  write(join5(outDir, "app", "layout.tsx"), tpl("layout.tsx.hbs", { hostname, targetOrigin: manifest.targetOrigin }));
  ...
  for (const spec of filteredRoutes) {
    ...
    write(join5(outDir, "app", ...segments, "route.ts"), tpl("route.ts.hbs", {
      method: spec.method,
      path: spec.path,
  ...
  write(join5(outDir, "prisma", "schema.prisma"), tpl("schema.prisma.hbs", { models }));
  write(join5(outDir, "lib", "replay.ts"), tpl("replay.ts.hbs", {}));
  write(join5(outDir, "next.config.js"), tpl("next.config.js.hbs", {}));
  write(join5(outDir, "package.json"), tpl("package.json.hbs", {
    siteName: hostname.replace(/\./g, "-"),
    hasPrisma: models.length > 0
  }));
  write(join5(outDir, "Dockerfile"), tpl("Dockerfile.hbs", {}));
  write(join5(outDir, "docker-compose.yml"), tpl("docker-compose.yml.hbs", {}));
  write(join5(outDir, ".env.example"), 'DATABASE_URL="file:./dev.db"\n');
  write(join5(outDir, "tsconfig.json"), tpl("tsconfig.json.hbs", {}));
  write(join5(outDir, "README.md"), tpl("README.md.hbs", {
    targetOrigin: manifest.targetOrigin,
    hostname,
...
CLONYFY v0.1`);
    ...
    async function notifyArtifact(event) {
      ...
      try {
        ...
        logger.warn(`  [OFFLOAD] ${event.relPath}: ${err.message}`);
...
async function regenerateCloneProject(outDir) {
  ...
  const fullManifest = { ...manifest, pages };
