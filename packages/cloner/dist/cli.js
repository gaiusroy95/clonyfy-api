...
program.command("clone <url>").description("Clone a website").option("-o, --out <dir>", "Output directory", "./output/site").option("-m, --max-pages <n>", "Max pages to crawl", "50").option("-d, --depth <n>", "Max link depth", "3").option("-c, --concurrency <n>", "Concurrent browser contexts", "1").option("--full-site", "Clone all discoverable same-origin pages (high page/depth budget)", false).option("--ignore-robots", "Skip robots.txt check", false).option("-v, --verbose", "Print DEBUG lines to console (all detail goes to log file regardless)", false).action(async (url, options) => {
  const fullSite = !!options.fullSite;
  const maxPages = fullSite ? Math.max(parseInt(options.maxPages, 10) || 50, parseInt(process.env.CLONYFY_FULL_SITE_MAX_PAGES || "10000", 10) || 1e4) : parseInt(options.maxPages, 10);
  const depth = fullSite ? Math.max(parseInt(options.depth, 10) || 3, parseInt(process.env.CLONYFY_FULL_SITE_DEPTH || "256", 10) || 256) : parseInt(options.depth, 10);
  await runClone({
    url,
